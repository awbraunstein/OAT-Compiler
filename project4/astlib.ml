...
let rec c_print_const_aux fmt c =
  let pps = pp_print_string fmt in
  begin match c with
...
let c_print_castin_aux fmt (_,fid) = 
  pp_print_string fmt   
    (if (fid = "length_of_array" ||
         fid = "string_of_array")
     then "(int*)"
     else if (fid = "array_of_string" ||
              fid = "print_string")
     then "(char*)"
     else "")

let c_print_castout_aux fmt (_,fid) = 
  pp_print_string fmt   
...
let rec c_print_exp_aux fmt level e =
  let pps = pp_print_string fmt in
  let this_level = prec_of_exp e in
  let is_shr = 
    begin match e with
  ...
  (match e with
  | Const  c -> c_print_const_aux fmt c
  | Lhs l -> c_print_lhs_aux fmt l
  | New (e1, (_,id), e2) ->
    pps "({ int ";  pps id;
    pps "; int size=";
    c_print_exp_aux fmt 0 e1;
    ...
    pps id;
    pps "+1]=";
    c_print_exp_aux fmt 0 e2;
    pps "; (int)(p+1); })"
  | Binop (o,l,r) ->
    pp_open_box fmt 0;
    c_print_exp_aux fmt this_level l;
    pp_print_space fmt ();
    pp_print_string fmt (c_string_of_binop o);
    pp_print_space fmt ();
    (let r_level = begin match o with
      | Times _ | Plus _ | And _ | Or _ -> this_level 
      | _ -> this_level + 1 
     end in
       c_print_exp_aux fmt r_level r);
    pp_close_box fmt ()
  | Unop (o,v) ->
    pp_open_box fmt 0;
    pp_print_string fmt (string_of_unop o);
    c_print_exp_aux fmt this_level v;
    pp_close_box fmt ()
  | Ecall (fid, es) ->
    c_print_castout_aux fmt fid;
    print_id_aux fmt fid;
    pps "(";
    print_list_aux fmt 
      (fun () -> pp_print_string fmt ","; pp_print_space fmt()) 
      (fun fmt -> fun e -> 
        c_print_castin_aux fmt fid;
        c_print_exp_aux fmt 0 e) es;
    pps ")"
  );
...
and c_print_lhs_aux fmt l =
  let pps = pp_print_string fmt in
  begin match l with
    | Var s -> print_id_aux fmt s
    | Index (l, e) -> 
      pps "((int*)";
      c_print_lhs_aux fmt l;
      pps ")[";
      c_print_exp_aux fmt 0 e;
      pps "]"
  end

let rec c_print_typ_aux fmt t =
  begin match t with
    | TBool _ -> pp_print_string fmt "int"
    | TInt _ -> pp_print_string fmt "int"
    | TString _ -> pp_print_string fmt "int"
    | TArray (t) -> c_print_typ_aux fmt t
  end

let rec c_print_init_aux fmt i =
  let pps = pp_print_string fmt in
  begin match i with
    | Iexp e -> c_print_exp_aux fmt 0 e
    | Iarray (_,is) ->
      pps (Printf.sprintf 
...
let mk_cvar : string -> string =
  let ctr = ref 0 in
    fun prefix -> let c = !ctr in ctr := !ctr + 1; prefix ^ (string_of_int c)

let c_print_vdecl_aux fmt {v_ty = t; v_id = id; v_init = i} =
  let tmp = mk_cvar "__ctmp__" in
  pp_open_hbox fmt ();
  c_print_typ_aux fmt t;
  pp_print_space fmt ();
  pp_print_string fmt tmp;
  pp_print_string fmt " = ";
  c_print_init_aux fmt i;
  pp_print_string fmt " ; ";
  c_print_typ_aux fmt t;
  pp_print_space fmt ();
  print_id_aux fmt id;
...
let rec c_print_block_aux fmt (vdecls, stmts) =
  if ((List.length stmts) > 0) then begin
    pp_open_vbox fmt 0;
    List.iter 
      (fun d -> 
        c_print_vdecl_aux fmt d; pp_print_string fmt ";"; pp_print_space fmt()) 
      vdecls;
    pp_close_box fmt ();
    print_list_aux fmt (fun () -> pp_print_space fmt ()) c_print_stmt_aux stmts
  end else begin
  if ((List.length vdecls) > 0) then begin
    pp_open_vbox fmt 0;
    print_list_aux fmt 
      (fun () -> pp_print_string fmt ";"; pp_print_space fmt()) 
      c_print_vdecl_aux vdecls;
    pp_print_string fmt ";";
    pp_close_box fmt ()
  end else ()
  end

and c_print_stmt_aux fmt s =
  let pps = pp_print_string fmt in
  let ppsp = pp_print_space fmt in
  begin match s with
    | Assign(l,e) ->
    ...
    | Scall (fid, es) ->
      ...
      pps ");"
    | If(e, s1, os2) ->
	pps "if ("; c_print_exp_aux fmt 0 e; pps ") ";
	c_print_stmt_aux fmt s1;
	begin match os2 with
	  | None -> ()
	  | Some s2 -> 
	      pps " else ";
	      c_print_stmt_aux fmt s2
	end
    | While(e, s) ->
	pps "while ("; c_print_exp_aux fmt 0 e; pps ") ";
	c_print_stmt_aux fmt s
    | For(vdecls, eo, so, body) ->
        pps "{"; pp_force_newline fmt ();
        print_list_aux fmt (fun () -> pps ";"; ppsp ()) c_print_vdecl_aux vdecls; 
        pps ";"; 
	pps "for (";
	  pps ";"; ppsp ();
	  begin match eo with
	    | None -> ();
	    | Some e -> c_print_exp_aux fmt 0 e;
	  end;
	  pps ";"; ppsp ();
	  begin match so with
	    | None -> ()
	    | Some s -> pps "({"; c_print_stmt_aux fmt s; pps "})"
	  end;
	pps ") ";
	c_print_stmt_aux fmt body;
        pps "}"
    | Block b ->
	  pps "{"; pp_force_newline fmt ();
	  pps "  "; pp_open_vbox fmt 0;
	  c_print_block_aux fmt b;
	  pp_close_box fmt (); pp_force_newline fmt ();
	  pps "}"
  end

let c_print_fdecl_aux fmt p (topt,(_,fid),args,b,eo) =
  let c_print_gvdecl_init_aux fmt {v_ty = t; v_id = id; v_init = i} =
    pp_open_hbox fmt ();
    print_id_aux fmt id;
    pp_print_space fmt ();
    pp_print_string fmt "=";
  ...
  (if fid = "program" then
     begin 
       ...
       List.iter
         (fun g ->
            match g with  
              | Gvdecl d -> c_print_gvdecl_init_aux fmt d; 
...
let c_print_fdecl_header_aux fmt p (topt,(_,fid),args,b,eo) =
  let pps = pp_print_string fmt in
  let ppsp = pp_print_space fmt in
  (match topt with
    | Some t -> c_print_typ_aux fmt t
...
let c_print_gvdecl_aux fmt {v_ty = t; v_id = id; v_init = i} =
  let rec _c_print_gvdecl_aux fmt i =
    let rec c_print_gconst_aux fmt c =
      begin match c with
    ...
    in
    begin match i with
      | Iexp (Const c) -> c_print_gconst_aux fmt c
...
let c_print_prog_aux fmt p =
  pp_open_vbox fmt 0;
  List.iter
    (fun g ->
      (match g with
	| Gvdecl d -> ()
        | Gfdecl f -> c_print_fdecl_header_aux fmt p f);
       pp_print_space fmt ()
    ) p;
  List.iter
    (fun g ->
      (match g with
	| Gvdecl d -> c_print_gvdecl_aux fmt d; pp_print_string fmt ";"
        | Gfdecl f -> c_print_fdecl_aux fmt p f);
       pp_print_space fmt ()
    ) p;
  pp_close_box fmt ()

let c_print_head fmt : unit =
  pp_open_vbox fmt 0;
...
let c_print_prog (p:Range.t prog) : unit =
  pp_open_hvbox std_formatter 0;
  c_print_head std_formatter;
  c_print_prog_aux std_formatter p;
  pp_close_box std_formatter ();
  pp_print_newline std_formatter ()

let c_string_of_prog (p:Range.t prog) : string =
  pp_open_hvbox str_formatter 0;
  c_print_head str_formatter;
  c_print_prog_aux str_formatter p;
  pp_close_box str_formatter ();
  flush_str_formatter ()

let c_run_prog (p:Range.t prog) (args:string) : string = 
...
(* AST to ML *)
let rec ml_string_of_const (c:Range.t const) : string =
  begin match c with
...
let rec ml_string_of_exp (e:Range.t exp) : string = 
  begin match e with 
    ...
    | New (e1, (_,id), e2) -> 
        Printf.sprintf "(New (%s, (norange, \"%s\"), %s))" 
