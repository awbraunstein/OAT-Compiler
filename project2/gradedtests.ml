...
let comp_test (arg:int) (e:Ast.exp) (ans:int) () : unit =
  let _ = if (!Platform.verbose_on) then
    Printf.printf "compiling: %s" (Ast.string_of_exp e)
...
(* X - (X - (X - 3)) *)
("med_parse11", (ast_test "X - (X - (X - 3))" (Binop (Minus,Arg,(Binop (Minus,Arg,(Binop (Minus,Arg,(Cint 3l)))))))));

(* X & 17 >> 2 *)
("med_parse12", (ast_test "X & 17 >> 2" (Binop (And,Arg,(Binop (Sar,(Cint 17l),(Cint 2l)))))));

(* X >> 2 & 17 *)
("med_parse13", (ast_test "X >> 2 & 17" (Binop (And,(Binop (Sar,Arg,(Cint 2l))),(Cint 17l)))));

(* X == 17 & X *)
("med_parse14", (ast_test "X == 17 & X" (Binop (And,(Binop (Eq,Arg,(Cint 17l))),Arg))));

(* X == 17 | X *)
("med_parse15", (ast_test "X == 17 | X" (Binop (Or,(Binop (Eq,Arg,(Cint 17l))),Arg))));

(* X * (X == X) - 17 & X *)
("med_parse16", (ast_test "X * (X == X) - 17 & X" (Binop (And,(Binop (Minus,(Binop (Times,Arg,(Binop (Eq,Arg,Arg)))),(Cint 17l))),Arg))));

(* X << (X == X) & (-17 >>> 2) *)
("med_parse17", (ast_test "X << (X == X) & (-17 >>> 2)" (Binop (And,(Binop (Shl,Arg,(Binop (Eq,Arg,Arg)))),(Binop (Shr,(Unop (Neg,(Cint 17l))),(Cint 2l)))))));

(* !~-!~X *)
("med_parse18", (ast_test "!~-!~X" (Unop (Lognot,(Unop (Not,(Unop (Neg,(Unop (Lognot,(Unop (Not,Arg))))))))))));

(* ~!-X *)
("med_parse19", (ast_test "~!-X" (Unop (Not,(Unop (Lognot,(Unop (Neg,Arg))))))));

(* X << (241 >> 3) >>> 1 *)
("med_parse20", (ast_test "X << (241 >> 3) >>> 1" (Binop (Shr,(Binop (Shl,Arg,(Binop (Sar,(Cint 241l),(Cint 3l))))),(Cint 1l)))));

]);]


let med_compiling_tests : suite = [
GradedTest("MediumCompiling", 15, [
(* X + X + X *)
...
(* X - (X - (X - 3)) *)
("med_compile11", (comp_test 341 (Binop (Minus,Arg,(Binop (Minus,Arg,(Binop (Minus,Arg,(Cint 3l))))))) 338));

(* X & 17 >> 2 *)
("med_compile12", (comp_test 341 (Binop (And,Arg,(Binop (Sar,(Cint 17l),(Cint 2l))))) 4));

(* X >> 2 & 17 *)
("med_compile13", (comp_test 341 (Binop (And,(Binop (Sar,Arg,(Cint 2l))),(Cint 17l))) 17));

(* X == 17 & X *)
("med_compile14", (comp_test 341 (Binop (And,(Binop (Eq,Arg,(Cint 17l))),Arg)) 0));

(* X == 17 | X *)
("med_compile15", (comp_test 341 (Binop (Or,(Binop (Eq,Arg,(Cint 17l))),Arg)) 341));

(* X * (X == X) - 17 & X *)
("med_compile16", (comp_test 341 (Binop (And,(Binop (Minus,(Binop (Times,Arg,(Binop (Eq,Arg,Arg)))),(Cint 17l))),Arg)) 324));

(* X << (X == X) & (-17 >>> 2) *)
("med_compile17", (comp_test 341 (Binop (And,(Binop (Shl,Arg,(Binop (Eq,Arg,Arg)))),(Binop (Shr,(Unop (Neg,(Cint 17l))),(Cint 2l))))) 682));

(* !~-!~X *)
("med_compile18", (comp_test 341 (Unop (Lognot,(Unop (Not,(Unop (Neg,(Unop (Lognot,(Unop (Not,Arg)))))))))) 0));

(* ~!-X *)
("med_compile19", (comp_test 341 (Unop (Not,(Unop (Lognot,(Unop (Neg,Arg)))))) (-1)));

(* X << (241 >> 3) >>> 1 *)
("med_compile20", (comp_test 341 (Binop (Shr,(Binop (Shl,Arg,(Binop (Sar,(Cint 241l),(Cint 3l))))),(Cint 1l))) 536870912));
...
("err4", (parse_error_test "X + (X + X))" (Failure "Parse error at stdin:[1.11-1.12].")));

("err5", (parse_error_test "((X)" (Failure "Parse error at stdin:[1.4-1.4].")));

("err6", (parse_error_test "((X +) Y" (Failure "Parse error at stdin:[1.5-1.6].")));

("err7", (parse_error_test "((X + ) X" (Failure "Parse error at stdin:[1.6-1.7].")));
("err8", (parse_error_test "" (Failure "Parse error at stdin:[1.0-1.0].")));
]);]

let hard_tests : suite = [
  GradedTest ("HardTests", 20, [
(* X & (3 - X) | (X * X * (X + 17) - X * 2) - X * 12 >>> 2 *)
("hard_parse0", (ast_test "X & (3 - X) | (X * X * (X + 17) - X * 2) - X * 12 >>> 2" (Binop (Or,(Binop (And,Arg,(Binop (Minus,(Cint 3l),Arg)))),(Binop (Shr,(Binop (Minus,(Binop (Minus,(Binop (Times,(Binop (Times,Arg,Arg)),(Binop (Plus,Arg,(Cint 17l))))),(Binop (Times,Arg,(Cint 2l))))),(Binop (Times,Arg,(Cint 12l))))),(Cint 2l)))))));

("hard_compile0", (comp_test 341 (Binop (Or,(Binop (And,Arg,(Binop (Minus,(Cint 3l),Arg)))),(Binop (Shr,(Binop (Minus,(Binop (Minus,(Binop (Times,(Binop (Times,Arg,Arg)),(Binop (Plus,Arg,(Cint 17l))))),(Binop (Times,Arg,(Cint 2l))))),(Binop (Times,Arg,(Cint 12l))))),(Cint 2l))))) 10405956));

(* X | (X & (X == (X < (X << (X + (X * ~X)))))) *)
("hard_parse1", (ast_test "X | (X & (X == (X < (X << (X + (X * ~X))))))" (Binop (Or,Arg,(Binop (And,Arg,(Binop (Eq,Arg,(Binop (Lt,Arg,(Binop (Shl,Arg,(Binop (Plus,Arg,(Binop (Times,Arg,(Unop (Not,Arg))))))))))))))))));

("hard_compile1", (comp_test 341 (Binop (Or,Arg,(Binop (And,Arg,(Binop (Eq,Arg,(Binop (Lt,Arg,(Binop (Shl,Arg,(Binop (Plus,Arg,(Binop (Times,Arg,(Unop (Not,Arg)))))))))))))))) 341));
