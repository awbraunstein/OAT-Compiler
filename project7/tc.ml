...
let rec find_parent (sigs:signature) (cid:Ast.cid) : cid option * signature = 
  match sigs with
    | [] -> (None, [])
    | (cid',(Some pid, _, _, _))::sigs2 -> 
        if cid = cid' then (Some pid, sigs2) 
        else (find_parent sigs2 cid) 
    | (cid',(None, _, _, _))::sigs2 -> (None, sigs2)
...
let rec subclass (sigs:signature) (cid1:Ast.cid) (cid2:Ast.cid) : bool = 
  if (cid1 = cid2) then
    (try (ignore (List.assoc cid1 sigs); true) with | Not_found -> false)
  else
    match find_parent sigs cid1 with
      | (None, _) -> false
      | (Some pid, sigs') -> subclass sigs' pid cid2
...
let subref (sigs:signature) (r1:Ast.ref) (r2:Ast.ref) : bool = 
  match (r1, r2) with
    | (RClass cid1, RClass cid2) -> subclass sigs cid1 cid2
    | (r1, r2) -> r1 = r2
...
let subtyping (sigs:signature) (t1:Ast.typ) (t2:Ast.typ) : bool = 
  match (t1, t2) with
    | (TRef r1, TRef r2) -> subref sigs r1 r2
    | (TNullable r1, TNullable r2) -> subref sigs r1 r2
    | (TRef r1, TNullable r2) -> subref sigs r1 r2 
  ...
  match e with
    ...
    | New (e1, (_,id), e2) ->
	  ...
	  | TInt ->
              ...
              let c = add_vdecl id TInt c in
              TRef (RArray (typecheck_exp c e2)) 
	  | _ -> failwith (report_typ_error (exp_info e1) TInt t1)
        end
    | Ctor ((info, cid), es) ->
...
let rec typecheck_stmt (c:ctxt) (s:Range.t stmt) : unit =
  ...
  | Cast (cid0, (info, id), e, st1, sto2) -> 
      ...
      begin match t with
	| TRef (RClass cid) -> 
          if not (wellformed_ref c.sigs (RClass cid0)) then
            failwith (report_wftyp_error info (TRef (RClass cid0)));
          if (not (subclass c.sigs cid0 cid)) then
            failwith (sprintf "%s: Cast exp must be a super type of %s." 
              (Range.string_of_range (exp_info e)) cid0);
          typecheck_option_stmt c sto2;
          let c = enter_scope c in
          let c = add_vdecl id (TRef (RClass cid0)) c in
          typecheck_stmt c st1
	| _ -> failwith (sprintf "%s was expected of class type." 
                 (Range.string_of_range (exp_info e)))
      end
  | While (e, st) ->
      typecheck c e TBool (typecheck_exp c e);
