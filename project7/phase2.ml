  ...
  begin match i with
	...
	let ao1 = compile_op slu a1 in
        let push_args = List.fold_left 
           ...
           [Push ao1] as3 in
        [Mov (edx, ao1); 
         Mov (edx, Ind {i_base = Some Edx; 
                        i_iscl = None; 
                        i_disp = Some (DImm (-4l))})] @
        push_args @ 
        [Call (Ind {i_base = Some Edx; 
                        i_iscl = None; 
                        i_disp = Some (DImm (Int32.of_int (4*i2)))})] @
        (match opa0 with
          | Some a0 -> let ao0 = compile_op slu a0 in [Mov (ao0, eax)]
          | None -> []) @ 
        [Add (esp, Imm (Int32.of_int (4 + 4 * List.length as3)))]
  end
...
let compile_cdecl 
  ...
  [Cunit.Data {Cunit.link=false;
               Cunit.label=this;
               Cunit.value=(match super_opt with
                 | None -> GInt32 0l
                 | Some super -> GLabelOffset (super, 4l))};
   Cunit.Data {Cunit.link=false;
               Cunit.label=tbl_lbl;
               Cunit.value=GLabels (List.map decorate_lbl tbl)};]
