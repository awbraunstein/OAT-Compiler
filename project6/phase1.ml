...
and compile_exp (c:ctxt) (e:Range.t exp) : ctxt * operand * stream = 
  ...
  | Ctor ((i, cid), es) ->
      (* 1) allocate a this pointer, and set all its contents to be zero
...
and compile_stmt c stmt : ctxt * stream =
    begin match stmt with
	  ...
	  *)
	  ...
	  let (c, ans, code_e) = compile_exp c e in
          let c = enter_scope c in
          let l = (lookup_cdecl cid c).cd_entry in
          let g = {Cunit.link=false;Cunit.label=l;Cunit.value=Cunit.GZero 4} in
          let (c, gl) = alloc (mk_tmp ()) None c in
          let (c, p) = alloc (mk_tmp ()) None c in
          let cond = mk_tmp () in
          let (c, r) = alloc cond (Some TInt) c in
	  let lcasta = X86.mk_lbl_hint "casta" in
	  let lcasta1 = X86.mk_lbl_hint "casta1" in
	  let lcasta2 = X86.mk_lbl_hint "casta2" in
	  let lcastb = X86.mk_lbl_hint "castb" in
	  let lcastb1 = X86.mk_lbl_hint "castb1" in
	  let lcastb2 = X86.mk_lbl_hint "castb2" in
          let (c, u) = alloc id (Some (TRef (RClass cid))) c in
          let check_cast = 
            code_e >::
              I (AddrOf (Slot gl, Global g, Imm 0l)) >::
              I (BinArith (Slot gl, Plus, Imm 4l)) >::
              I (BinArith (Slot u, Move, ans)) >::
              I (BinArith (Slot p, Move, ans)) >::
              I (BinArith (Slot p, Minus, Imm 4l)) >::
              I (Load (Slot p, Slot p)) >::
            L lcasta >::
              J (If (Slot p, Eq, Imm 0l, lcasta1, lcastb1)) >:: 
            L lcasta1 >::
              I (BinArith (Slot r, Move, Imm 0l)) >::
              J (Jump lcastb) >::
            L lcastb1 >::
              J (If (Slot p, Eq, Slot gl, lcasta2, lcastb2)) >:: 
            L lcasta2 >::
              I (BinArith (Slot r, Move, Imm 1l)) >::
              J (Jump lcastb) >::
            L lcastb2 >::
              I (BinArith (Slot p, Minus, Imm 4l)) >::
              I (Load (Slot p, Slot p)) >::
          ...
          let (c, if_code) = 
            compile_stmt c 
              (Ast.If(LhsOrCall (Lhs (Var (Range.norange, cond))), stmt1, 
                ostmt2)) in
          let c = leave_scope c in
            (c, check_cast >@ if_code)
...
let compile_ctor c cid cidopt ((args, es, cis, b):Range.t Ast.ctor) 
  ...
  let c = 
    ...
    args 
  in
  let (c, u) = alloc (mk_tmp ()) None c in
  let (c, u') = alloc (mk_tmp ()) None c in
  let (c, super_code) = match cidopt with
    | None -> (c, [])
    | Some super ->
        compile_stmt c 
          (Assign (Var (Range.norange, "_this"),
            (LhsOrCall (Ast.Call (Func ((Range.norange, mk_ctor_name super), 
              (LhsOrCall (Lhs (Var (Range.norange, "_this"))))::es))))))
  in
  let (c, cinit_code) = compile_cinits c (cinit_class_name cid::cis) in
  let check_code = 
    List.fold_left
      (fun accum -> fun (_, (n, t)) ->
        match t with
	  | TRef _ -> 
              let lthen = X86.mk_lbl_hint "then" in
              let lmerge = X86.mk_lbl_hint "merge" in
              accum >:: I (BinArith (Slot u, Move, Arg 0))
              >:: I (BinArith (Slot u, Plus, Imm (Int32.of_int (4 * n))))
              >:: I (Load (Slot u, Slot u))
              >:: J (If (Slot u, Eq, Imm 0l, lthen, lmerge))
              >:: L lthen
              >:: I (Call (None, "oat_abort", [Imm 1l]))
              >:: J (Jump lmerge)
              >:: L lmerge
	  | _ -> accum
      ) ([L (l)] >@ (super_code >@ cinit_code)) (lookup_this_cfields c)
  in
  let g = {Cunit.link=false;Cunit.label=lookup_this_class_entry c;
    Cunit.value=Cunit.GZero 4}
  in
  let vtbl_code = check_code >:: 
     I (BinArith (Slot u, Move, Arg 0)) >::
     I (BinArith (Slot u, Minus, Imm 4l)) >::
     I (AddrOf (Slot u', Global g, Imm 0l)) >::
     I (BinArith (Slot u', Plus, Imm 4l)) >::
     I (Store (Slot u, Slot u'))
  in
  let (c, block_stream) = compile_block c b false in
  let code = vtbl_code >@ block_stream >:: J (Ret (Some (Arg 0))) in
  let c = leave_scope c in
  let (n, c) = clear_args c in
